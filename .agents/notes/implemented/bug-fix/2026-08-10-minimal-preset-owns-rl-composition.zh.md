...
随附的 Web `minimal` preset 是 RL agent 组合在 Web 中的唯一所有者。它声明 entry 本地的 PTY 注册表与本地后端、带 RL 环境描述且超时为 300 秒的持久 `bash`，以及 `str_replace_editor`。工具呈现仍由部署选择。后续的[裸双工具运行时决策](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.md)取代了本记录最初的压缩与文件系统提供方选择：当前 preset 挂载 entry 本地的 `fs-local` 提供方，不挂载压缩后端。编辑器不接受 `requireAbsolutePath` 设置，因为要求绝对路径是它的无条件约定。持久 bash 栈仅限 POSIX——`subprocess-local` 仅实现 linux 与 darwin 的终端检查——因此 win32 会话将其禁用，并挂载宿主 shell 执行器之上的一次性 pwsh 工具，与 standard、code、cordis 预设应用于其 shell 工具行的互逆平台门控相同。
...
**为 win32 构建持久 pwsh PTY 栈。** 被拒绝，因为持久 PTY 后端没有 win32 终端检查器，且交付的 Windows shell 策略是一次性 pwsh 工具；因此 win32 的 minimal 会话镜像 standard 预设的 shell，而不是发明新的终端后端。

## 后果

Web RL 提示词固定不变，不能通过环境覆盖；独立 JSON-RPC 提示词由部署选择。Web preset 与独立 JSON-RPC 示例分别在各自的启动路径声明相同的双工具约定。模型只看到一个 shell 与 `str_replace_editor`——POSIX 上为按 agent 隔离状态的持久 `bash`，win32 上为按调用隔离状态的一次性 `pwsh` 工具。Web preset 为自身的 PTY 与裸文件系统服务实例承担开销，其他 preset 无需承担。持久 shell 后端需要受支持的 POSIX 终端基础环境，因此 win32 会话以宿主 shell 执行器之上的一次性 pwsh 工具替代。
