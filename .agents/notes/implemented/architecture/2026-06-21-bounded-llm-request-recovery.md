...
The default policy provides bounded recovery from transient failures of the same explicit provider/model request. Provider or model failover and response splicing remain outside this base recovery policy; the opt-in [logical model policy adapter](2026-08-17-logical-model-policy-routing.md) owns a separate pre-output candidate-selection path. Semantic output repair has no current consumer.
...
`ctx.llm.stream()` remains the raw one-attempt waterfall for physical adapters. Direct callers such as compaction summarization receive the structured failure but do not gain automatic same-route retry, because they have no agent step boundary or general durable place to separate attempts. The opt-in [logical model policy adapter](2026-08-17-logical-model-policy-routing.md) is a separate buffering consumer that may select another physical route before emitting a model block; this base decision adds no such helper.
...
- Automatic provider or model failover inside the base recovery plugin. Requests already select one explicit physical provider and model, and the provider registry deliberately has one adapter owner per provider; the separate opt-in logical policy adapter owns its pre-output candidate selection.
...
- **Add logical routes, capability matrices, and failover selection to `dsh-llm`** — rejected because the core requests name physical provider and model explicitly, one adapter owns each provider, and deployment policy does not belong in the base runtime. The separate opt-in logical policy adapter owns this behavior for deployments that provide the needed capability metadata.
...
- Physical-adapter `ctx.llm.stream()` callers remain single-attempt and receive the same structured failure facts; the separate logical policy adapter may select another physical route before content begins.
