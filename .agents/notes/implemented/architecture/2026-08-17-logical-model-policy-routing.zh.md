...
逻辑模型声明 `input`、`maxTokens`、`reasoning.allowed`、`reasoning.default`、`serviceTier` 和有序路由。配置会拒绝重复物理候选，以及不在逻辑允许列表中的默认思考档位。加载期验证要求每个声明的图片能力或非 off 思考能力至少存在于一个物理候选；请求期还会针对实际图片和思考请求再次筛选候选。

`serviceTier: fast` 会在复用的 pi-ai 适配器中映射为 `service_tier: priority`。其他支持的等级会发送到 OpenAI 兼容的 pi-ai API；不兼容的协议会以 `UNSUPPORTED_OPTION` 失败，而不是丢弃字段。这个 wire 专用映射留在 `dsh-llm-pi-ai`，由它为组合导出 profile schema、profile 解析器、service-tier 词汇和按请求解析的凭据解析器。
...
包测试覆盖逻辑元数据、思考能力交集、图片能力筛选、内容前故障切换和安全的回放请求映射。真实 Loader 组合会启动测试专用 `cordis.yml`，注册逻辑提供方，验证 Fast wire 字段和逻辑 token 上限，并在两个本地 mock 网关之间执行故障切换。pi-ai 适配器测试覆盖公开 profile 导出和 OpenAI 兼容请求路径上的 Fast 映射。

## 考虑过的替代方案

- **把逻辑模型和故障切换放进 `LlmRuntime`**——不予采纳，因为核心 seam 应保持物理路由所有权，并且在没有部署专用策略或 pi-ai 的情况下仍应可用。
- **把 service tier 加入共享的 `GenerateOptions` wire 词汇**——不予采纳，因为该选项是提供方专用的；逻辑策略通过 pi-ai payload hook 映射，并拒绝无法承载它的协议。
