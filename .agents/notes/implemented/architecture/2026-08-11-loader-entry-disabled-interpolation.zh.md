...
行可以按平台或环境门控自身；错误的表达式在启动时响亮失败。其余元数据字段保持字面值，门禁继续拒绝那里的表达式——`disabled` 上的 postmortem-0002 隐患以「求值」而非「禁止」关闭。Windows shell 栈的切换从启动器注入的 patch 层移到 base bundle 自身的行上：win32 挂载受限 pwsh 栈，POSIX 携带被禁用的 pwsh 行，同一份 patch 文件服务两种阵容——[Windows 默认 pwsh](../feature/2026-08-01-windows-pwsh-default.md) note 的层机制已被取代。shell 工具行遵循与其他预设声明行相同的 one-plane 规则：web-app overlay 禁用 host 面的 `tool-bash`/`tool-pwsh` 行，预设以互逆的平台门控声明两者，因此任一宿主的每个会话都可以按预设丢弃或替换 shell 工具。`minimal` 预设也应用同样的互逆门控：其持久 bash PTY 栈行（`pty`、`terminal-bash`、`persistent-bash`）携带 `disabled: !!js process.platform === 'win32'`，`tool-pwsh` 行仅在 win32 挂载，因此 win32 的 minimal 会话使用宿主 shell 执行器之上的一次性 pwsh 工具，而 POSIX 保留持久 bash 栈。
