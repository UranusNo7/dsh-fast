...
默认策略的目标是从同一个显式提供方／模型请求的暂时性失败中进行有界恢复。提供方或模型故障转移与响应拼接仍属于本基础恢复策略之外；可选的[逻辑模型策略适配器](2026-08-17-logical-model-policy-routing.md)拥有独立的内容前候选选择路径。语义输出修复目前没有消费方。
...
对于物理适配器，`ctx.llm.stream()` 仍是原始的单次尝试 waterfall。压缩（compaction）摘要等直接调用方会收到结构化失败，但不会自动获得同一路由重试，因为它们没有 agent 步骤边界，也没有可供分隔尝试的通用持久位置。可选的[逻辑模型策略适配器](2026-08-17-logical-model-policy-routing.md)是独立的缓冲消费方，可以在产生模型块前选择另一个物理路由；本基础决策不增加此类辅助函数。
...
- 在本基础恢复插件内自动进行提供方或模型故障转移。请求已显式选择一个物理提供方和模型，提供方注册表也有意规定每个提供方只由一个适配器负责；独立的可选逻辑策略适配器拥有其内容前候选选择。
...
- **在 `dsh-llm` 中增加逻辑路由、能力矩阵和故障转移选择**：拒绝采用，因为核心请求显式指定物理提供方和模型，每个提供方由一个适配器负责，部署策略不属于基础 runtime。独立的可选逻辑策略适配器为提供所需能力元数据的部署拥有这项行为。
...
- 物理适配器的 `ctx.llm.stream()` 直接调用方仍只尝试一次，并收到相同的结构化失败事实；独立的逻辑策略适配器可以在内容开始前选择另一个物理路由。
