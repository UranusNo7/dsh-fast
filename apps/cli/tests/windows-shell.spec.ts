...
describe('shipped agent presets gate both shell tools by platform', () => {
  ...
  it('minimal gates its PTY stack to POSIX and mounts one-shot pwsh on win32', () => {
    const entries: unknown = yaml.load(
    ...
    const find = (id: string, rows: readonly unknown[]): Record<string, unknown> => {
      const row = rows.find((entry): entry is Record<string, unknown> => (
        typeof entry === 'object' && entry !== null && (entry as Record<string, unknown>).id === id
      ))
      if (row === undefined) throw new TypeError(`minimal preset must mount ${id}`)
      return row
    }
    // The one-shot pwsh tool is win32-only; the PTY stack is its POSIX twin.
    const pwsh = find('tool-pwsh', entries)
    expect(pwsh.disabled).toMatchObject({ __jsExpr: expect.any(String) as string })
    const pwshExpression = (pwsh.disabled as { __jsExpr: string }).__jsExpr
    expect(Boolean(evaluate({ process: { platform: 'win32' } }, pwshExpression)), 'tool-pwsh on win32').toBe(false)
    expect(Boolean(evaluate({ process: { platform: 'linux' } }, pwshExpression)), 'tool-pwsh on linux').toBe(true)
    // One-shot tool-bash stays absent: the POSIX shell is the PTY stack.
    expect(entries.some(entry => (
      typeof entry === 'object' && entry !== null && (entry as Record<string, unknown>).id === 'tool-bash'
    )), 'tool-bash must be absent from minimal').toBe(false)
    const shell = find('persistent-shell', entries)
    const stack = Array.isArray(shell.config) ? shell.config : []
    for (const id of ['pty', 'terminal-bash', 'persistent-bash']) {
      const row = find(id, stack)
      expect(row.disabled).toMatchObject({ __jsExpr: expect.any(String) as string })
      const expression = (row.disabled as { __jsExpr: string }).__jsExpr
      expect(Boolean(evaluate({ process: { platform: 'win32' } }, expression)), `${id} on win32`).toBe(true)
      expect(Boolean(evaluate({ process: { platform: 'linux' } }, expression)), `${id} on linux`).toBe(false)
    }
  })
})
