...
import { FirecrawlFetchProvider } from '@deepseek-ai/dsh-web-fetch-firecrawl'

const TEST_API_KEY = 'redirect-test-key'
const TEST_URL = 'https://private.example/secret'
...
describe('FirecrawlFetchProvider redirect policy', () => {
  it.each([301, 302, 303, 307, 308])('rejects HTTP %i before contacting Location', async (status) => {
    ...
    await expect(provider.fetch({ url: TEST_URL }))
