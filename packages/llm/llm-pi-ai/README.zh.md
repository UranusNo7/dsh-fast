...
包根入口导出 Cordis 插件约定、`PiAiAdapter`、已解析 profile schema、profile 解析、凭据解析与 `supportedProtocols()`，供策略适配器组合使用；提供方构造、回放转换和流转换仍保留在包内部。
