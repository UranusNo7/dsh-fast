...
import type { PiAiServiceTier, ResolvedPiAiProviderProfile } from './config.ts'
...
function wireServiceTier(serviceTier: PiAiServiceTier): string {
  return serviceTier === 'fast' ? 'priority' : serviceTier
}

/** Add the provider field without changing non-OpenAI payloads. */
function payloadWithServiceTier(payload: unknown, serviceTier: PiAiServiceTier): Record<string, unknown> | undefined {
