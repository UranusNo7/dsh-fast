...
The package root exposes the Cordis plugin contract, `PiAiAdapter`, the resolved profile schema, profile resolution, credential resolution, and `supportedProtocols()` for composing policy adapters; provider construction, replay conversion, and stream conversion remain package-internal.
...
`serviceTier` is an explicit route request option for OpenAI-compatible pi-ai APIs. A request-level `GenerateOptions.serviceTier` overrides the profile's static `serviceTier` for that call; omitting it preserves the profile value. `fast` is the Harness alias for the current OpenAI `priority` value; `default`, `auto`, `flex`, `scale`, and `priority` pass through unchanged. The adapter injects `service_tier` through pi-ai's `onPayload` hook and rejects a non-OpenAI model with `UNSUPPORTED_OPTION` instead of silently dropping the field.
