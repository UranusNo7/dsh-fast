...
describe('PiAiAdapter provider routing', () => {
  ...
  it('maps Fast to the current OpenAI priority service tier', async () => {
    const server = await mockServer([{ events: textEvents }])
    const ctx = await harness(server.url, { serviceTier: 'fast' })
...
describe('provider profile lifecycle', () => {
  it('exports composition helpers but keeps conversion helpers private', () => {
    expect(LlmPiAi).toHaveProperty('resolveProfiles')
    expect(LlmPiAi).toHaveProperty('ProviderProfileConfig')
    expect(LlmPiAi).toHaveProperty('resolvePiAiApiKey')
    for (const helper of [
      'toPiContext',
      'toPiReplayState',
      'toPiAssistant',
