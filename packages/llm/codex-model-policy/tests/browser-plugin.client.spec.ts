/**
 * codex-model-policy browser half on a real cordis Context with fake command/slots/
...
async function bench() {
  ...
  let fast = { active: false, available: true }
  const calls = { models: 0, select: 0 }
  ctx.provide('connection', { api: { sessions: {
    models: () => {
      calls.models += 1
      return Promise.resolve({
        result: { ok: true as const, value: { current, fast, routable, groups: GROUPS, failures: [] } },
      })
    },
    selectModel: (payload: { provider: string; model: string; reasoningEffort?: string; fast?: boolean }) => {
      calls.select += 1
      if (payload.fast !== undefined) fast = { ...fast, active: payload.fast }
      current = {
      ...
      return Promise.resolve({ result: { ok: true as const, value: { selected: current, fast } } })
    },
  } } })
...
describe('codex-model-policy dual entry', () => {
  ...
  it('the seat Fast toggle updates the shared session state through the Host', async () => {
    const b = await bench()
    b.mint('s1')
    const seatFace = b.seat().inject!(sid('s1'))
    await b.ctx.modelDirectories.directoryFor(sid('s1')).load()
    expect(await seatFace.toggleFast(true)).toBe(true)
    expect(seatFace.directory.getSnapshot().fast).toEqual({ active: true, available: true })
    expect(b.calls.select).toBe(1)
    expect(await seatFace.toggleFast(false)).toBe(true)
    expect(seatFace.directory.getSnapshot().fast).toEqual({ active: false, available: true })
  })
