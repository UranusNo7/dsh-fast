...
describe('codex-model-policy Fast session mode', () => {
  it('provides a Host controller that enables and disables Fast on a GPT logical model', async () => {
    ...
    try {
      ...
      expect(controller.getFast(agent, selection)).toEqual({ active: false, available: true })
  ...
  it('keeps Fast available for a legacy physical route selection', async () => {
    const { ctx, agent } = await mount()
    try {
      const controller = ctx.get('modelPolicy') as ModelPolicy.ModelPolicySessionController
      ...
      expect(controller.getFast(agent, selection)).toEqual({ active: false, available: true })
      expect(controller.setFast(agent, selection, true)).toEqual({ active: true, available: true })
      agent.options.provider = 'primary'
      agent.options.model = 'gpt'
      await expect(request(ctx, agent)).resolves.toMatchObject({
        provider: 'primary',
        model: 'gpt',
        serviceTier: 'fast',
  ...
  it('rejects Fast for non-GPT models and enforces the rule in request middleware', async () => {
    ...
    try {
      ...
      expect(() => controller.setFast(agent, selection, true)).toThrow('Fast mode is available only for GPT logical models.')
  ...
  it('persists the Fast choice in request headers across loop steps', async () => {
    ...
    try {
      ...
      controller.setFast(agent, { provider: 'model-policy', model: 'gpt-5.6' }, true)
      agent.followup(createUserMessage({ content: [{ type: 'text', text: 'second' }], source: { kind: 'user' } }))
      await waitForIdle(ctx, agent)
      expect(server.paths).toHaveLength(2)

      controller.setFast(agent, { provider: 'model-policy', model: 'gpt-5.6' }, false)
