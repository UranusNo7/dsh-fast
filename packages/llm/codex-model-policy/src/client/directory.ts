...
/** Session-local Fast state; mirrors Host's model-policy/fast event. */
...
export interface ModelDirectoryState {
  ...
  /** Session-local Fast state; null before the first load or without the policy plugin. */
...
export class ModelDirectory {
  ...
  async load(): Promise<SessionModels> {
    ...
    const { current, routable, groups, failures } = result.value as SessionModels
    const fast = (result.value as { fast?: ModelPolicyFastState | null }).fast
    this.store.update((s) => {
      s.current = current
      s.fast = fast ?? null
      s.routable = routable
 ...
 */
  async select(selection: ModelSelection): Promise<void> {
    ...
    this.store.update((s) => {
      s.current = (result.value as { selected: ModelSelection }).selected
      s.fast = (result.value as { fast?: ModelPolicyFastState | null }).fast ?? null
      s.routable = true
      s.status = 'ready'
      s.error = null
    })
   ...
   */
  async setFast(active: boolean): Promise<void> {
    this.assertAvailable()
    const current = this.store.getSnapshot().current
    if (current === null) throw new Error('cannot set Fast before the model directory loads')
    const generation = ++this.generation
    this.store.update((s) => { s.status = 'selecting'; s.error = null })
    const { result } = await this.sessions.selectModel({
      sessionId: this.sessionId,
      ...
      fast: active,
    } as unknown as Parameters<IApiClient['sessions']['selectModel']>[0])
    if (this.disposed || generation !== this.generation) {
      if (!result.ok) throw new Error(`${result.error.code}: ${result.error.message}`)
      return
    }
