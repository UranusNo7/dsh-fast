 ...
 */
import type {
  IApiClient, ModelCatalogFailure, ModelPolicyFastState, ModelProviderGroup, ModelSelection, SessionId, SessionModels,
...
export class ModelDirectory {
  ...
  async load(): Promise<SessionModels> {
    ...
    const { current, fast, routable, groups, failures } = result.value
    this.store.update((s) => {
      s.current = current
      s.fast = fast ?? null
      s.routable = routable
