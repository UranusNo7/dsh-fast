...
export class ModelDirectoryResolver extends Service {
  ...
  directoryFor(sessionId: SessionId): ModelDirectory {
    ...
    if (actx === undefined) throw new Error(`codex-model-policy: session "${String(sessionId)}" resolved no scope`)
    ...
    if (conversation !== undefined) {
      ...
      }, 'codex-model-policy: composer block')
    }
    actx.effect(() => () => {
      directory.dispose()
      live.directories.delete(sessionId)
    }, 'codex-model-policy: session directory')
    return directory
  }
}
