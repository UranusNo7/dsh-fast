...
async function setupSandboxed(withApproval = false, mode: SandboxMode = 'read-only') {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('sandbox escalation through ctx.approval', () => {
  ...
  it('a danger-full-access standing policy runs sandbox_permissions + an EMPTY justification with no escalation', async () => {
    // The default danger-full-access policy comes from the SandboxPolicyService
    // deployment default (no per-session override), so the call's effective mode
    // is the STANDING policy — the compat fields must be inert, not escalated.
    const { ctx, bash } = await setupSandboxed(false, 'danger-full-access')
    const agent = sandboxAgent(undefined, ctx)
    ctx.agents.register(agent)
    const result = await call(ctx, 'pwsh', {
      command: 'Write-Output ok',
      description: 'compat fields',
      sandbox_permissions: 'danger-full-access',
      justification: ' ',
    }, agent)
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected the compat fields to be inert under danger-full-access')
    // No escalation was asked: the call ran under the standing danger-full-access policy.
    expect(bash.requests[0]?.sandboxPolicy?.mode).toBe('danger-full-access')
    expect(bash.modes).toEqual(['danger-full-access'])
  })

  it('keeps the strict pairing failure when the effective mode is a confined workspace-write', async () => {
    const { ctx } = await setupSandboxed()
    const agent = sandboxAgent('workspace-write')
    const malformed = await call(ctx, 'pwsh', {
      command: 'Write-Output ok',
      description: 'd',
      sandbox_permissions: 'workspace-write',
      justification: ' ',
    ...
    expect(text(malformed)).toContain('invalid justification: expected a non-empty sentence')
  })
