...
| `sandbox_permissions` | string enum | Advertised only when a sandboxing executor is mounted (`ctx.shell.sandboxMode` defined). The wider sandbox mode for a one-shot retry of a command the sandbox just denied — the narrowest wider mode that suffices, requiring `justification` and user approval through `ctx.approval` BEFORE execution. A non-widening or unapprovable request fails closed without running anything. Confined effective modes (`read-only`, `workspace-write`) keep this strict pairing and strict-widening contract. Under an effective `danger-full-access` standing policy the fields are inert compatibility fields — the escalation request already granted full access cannot be wider — so they are ignored and the command runs under the standing policy with no approval prompt, even when `justification` is empty. |
| `justification` | string | Required with `sandbox_permissions`: one sentence for the user explaining why this exact command needs the wider access. Ignored (pairing not enforced) when the call's effective mode is already `danger-full-access`. |
