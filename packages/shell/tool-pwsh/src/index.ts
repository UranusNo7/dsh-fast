 ...
 * exit-status parse from `@deepseek-ai/dsh-shell`. One escalation exception
 * keeps this from being perfectly call-for-call with bash: confined modes
 * escalate strictly (paired `sandbox_permissions` + non-empty
 * `justification`, strictly-wider approval), while an effective
 * `danger-full-access` standing policy treats outer-materialized escalation
 * fields as inert compatibility fields — the call runs under the standing
 * policy with no approval prompt.
 *
...
function validatePwshArgs(args: PwshToolArgs): void {
  ...
  // Escalation pairing is judged in execute against the call's effective
  // mode, because `danger-full-access` treats those fields as inert
  // compatibility fields rather than an escalation request.
}
/* jscpd:ignore-end */
...
export function apply(ctx: Context, config: Config = {}): void {
   ...
   * tool-plugin load. Confined calls with escalation fields always reach
   * this resolver; under an effective `danger-full-access` standing policy
   * `execute` treats the fields as inert compatibility fields and never
   * calls it.
   */
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      // Under an effective `danger-full-access` policy the standing grant already
      // covers full access, so `sandbox_permissions`/`justification` are inert
      // compatibility fields: strict pairing is skipped (a model-supplied empty
      // justification must not fail the call) and no approval prompt is raised —
      // there is nothing to widen. Confined modes keep the shared strict pairing
      // and strict-widening escalation.
      if (effectiveMode !== 'danger-full-access') {
        validateEscalationArgs(args.sandbox_permissions, args.justification)
      }
      // Keep the pairing condition inline so TypeScript narrows the fields to
      // `string` in the granted branch (a folded boolean would not carry that).
      const approvedMode = (
